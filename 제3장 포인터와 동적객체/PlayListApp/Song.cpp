...
{
    // 생성될 때마다 1씩 증가하므로 모든 Song 객체는 유일한 id를 갖게됨.
    id_counter = (id_counter + 1) % MAX_SONG_ID;
}

Song::Song(string ti, string ar, string al, string mv) : id(id_counter), title(ti), artist(ar), album(al), mv_url(mv)
{
    id_counter = (id_counter + 1) % MAX_SONG_ID;
}
...
{
    // Song 이 가진 플리에서 내가 삭제할 플리의 이름을 찾음
    // 해당 칸을 erase
